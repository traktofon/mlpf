      subroutine h5o2_init(zlog,rlog,d,m)
!        zlog : true if transform z -> z/(R-2d)
!        zlog : true if R is valence coordinate
!        d    : minimium distance of the proton to edges of R
!        m    : mass of the external hydrogens
         implicit none

         logical, intent(in)       :: zlog
         logical, intent(in)       :: rlog
         real (kind=8), intent(in) :: d
         real (kind=8), intent(in) :: m

         logical                   :: zz,rval
         real (kind=8)             :: dmin,kmin,mh,mo,mwat
         common/trans/zz,rval,dmin,kmin,mh,mo,mwat
         ...
         write(6,*) " "
         write(6,*) "*** H5O2+ Potential Initialized ***"
         if (zz) then
            write(6,*) "   - Z is transformed to Z/(R-2d)"
         end if
         if (rval) then
           write(6,*) "   - R is Valence vector"
         else
           write(6,*) "   - R is Jacobi vector"
         end if
         write(6,*) "   -         Dmin= ",dmin
         write(6,*) "   - Proton mass = ",mh
         write(6,*) " "
         write(6,*) " "
        ...
        subroutine potpoly(v,q)
        implicit none

        real (kind=8), intent(in)  :: q(0:14)
        real (kind=8), intent(out) :: v
        real (kind=8)              :: xm(0:6,0:2)
        ...
        subroutine dippoly(dp,q)
        implicit none

        real (kind=8), intent(in)  :: q(0:14)
        real (kind=8), intent(out) :: dp(3)
        real (kind=8)              :: xm(0:6,0:2)
...
! sunroutine to be handled directed by f2py, this is just a wrapper around
! calcpot offering a 21 pos vector, instead of a 7x3 matrix, interface.
        subroutine pot(v,cart)
        implicit none

        integer                                      :: i,j,k
        double precision, dimension(21),intent(in)   :: cart
        double precision, intent(out)                :: v
        double precision, dimension(7,3) :: cartm
        ...
        do i=1,7
          ...
          enddo
        enddo
        call calcpot(v,cartm)
        return
        end

! sunroutine to be handled directed by f2py, this is just a wrapper around
! calcdip offering a 21 pos vector, instead of a 7x3 matrix, interface.
        subroutine dip(d,cart)
        implicit none
        ...
        do i=1,7
          ...
          enddo
        enddo
        call calcdip(d,cartm)
        return
        end


      SUBROUTINE q2x(qq,xm)
      IMPLICIT NONE
      real (kind=8), intent(in)      :: qq(0:14)
      real (kind=8), intent(out)     :: xm(0:6,0:2)
      real (kind=8)                  :: q(0:14)
      real (kind=8), dimension (0:2) :: xHp,xOa,xH1a
      real (kind=8), dimension (0:2) :: xH2a,xCMa,xOb,xH1b,xH2b,xCMb
      real (kind=8)                  :: R(0:2,0:2)
...
!   # We do not want to mess with the input vector qq
       q(:)=qq(:)
...
!   # Central proton in BFA
      ...
      xHp(1) = q(1)
      xHp(2) = q(2)
!     CALL eulerMatrix(R,-0.5d0*q(4),0.d0,0.d0)
!     xHp = matmul(R,xHp)
!   # Wat A
...
!   # Shift atoms so that the oxygen lies at (0,0,0)_BFA
      if (rval) then
        ...
        xCMa = (mo*xOa + mh*xH1a + mh*xH2a)/(mwat)
      end if
      xOa = xOa - xCMa
      xH1a = xH1a - xCMa
      xH2a = xH2a - xCMa
!   # Apply Euler rotations
      CALL eulerMatrix(R,0.d0,q(8),q(9))
      xOa = matmul(R,xOa)
      xH1a = matmul(R,xH1a)
      xH2a = matmul(R,xH2a)
!   # Displace watA by 0.5*R to the left
      xOa(2)  = xOa(2)  - 0.5d0*q(3)
      xH1a(2) = xH1a(2) - 0.5d0*q(3)
      xH2a(2) = xH2a(2) - 0.5d0*q(3)
!   # Wat B
...
!   # Shift atoms so that the oxygen lies at (0,0,0)_BFB
      if (rval) then
        ...
        xCMb = (mo*xOb + mh*xH1b + mh*xH2b)/(mwat)
      end if
      xOb = xOb - xCMb
      xH1b = xH1b - xCMb
      xH2b = xH2b - xCMb
!   # Apply Euler rotations
      CALL eulerMatrix(R,q(4),q(13),q(14))
      xOb = matmul(R,xOb)
      xH1b = matmul(R,xH1b)
      xH2b = matmul(R,xH2b)
!   # Displace watB by 0.5*R to the right
      xOb(2)  = xOb(2)  + 0.5d0*q(3)
      xH1b(2) = xH1b(2) + 0.5d0*q(3)
      xH2b(2) = xH2b(2) + 0.5d0*q(3)
...
!*******************************************************************
! Pretransformation routines
      SUBROUTINE qcos2q(q,qout)
      IMPLICIT NONE
      real (kind=8), intent(in)  :: q(0:14)
      real (kind=8), intent(out) :: qout(0:14)
...
!     Change the definition of z
      q(2) = q(2)*(q(3)-kmin)
      return
      END SUBROUTINE
!*******************************************************************

      SUBROUTINE eulerMatrix(r,a,b,c)
      IMPLICIT NONE
      real (kind=8), intent(in)  :: a,b,c
      real (kind=8), intent(out) :: r(0:2,0:2)
      real (kind=8) :: Am(0:2,0:2)
      real (kind=8) :: Bm(0:2,0:2)
      real (kind=8) :: Cm(0:2,0:2)
...
!
!
!  Permanent contact:  Joel M. Bowman    E-mail: bowman@euch4e.chem.emory.edu
!------------End of Note----------------------------------------------
!********************************************************************************
